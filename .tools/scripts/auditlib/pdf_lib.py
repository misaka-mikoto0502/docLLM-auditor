...
设计目标(可复用/可泛化)：
  ...
  * 将 页文本 -> 清洗文本 缓存为 json，避免每次重解析数千页大 PDF。
...
DEFAULT_FOOTER_MARKS = ("版权所有", "API 参考", "文档版本", "API 参考")

def clean_page_text(text: str) -> list[str]:
    """清洗单页文本：去掉页眉/页脚/页码，返回非空、裁剪后的行列表。

    通用规则(不针对具体产品定制，仅针对这类版式的云厂商 API 文档)：
      - 移除单独的页码行(整行仅数字)
      - 移除页脚段：从含“版权所有”的行往后(版权/章节名/页码尾块)
      - 移除“文档版本 ... / 日期”行与 “产品名/API 参考” 头
    """
    lines = text.split("\n")
    out = []
    for ln in lines:
        ...
        if re.match(r"^文档版本\s*\d", s):
            continue
        if re.fullmatch(r"(产品名|API参考|API 参考)", s):
            continue
        out.append(s)
...
# 实测(某产品, h=842)：页眉 y≈42-67、页脚 y≈773-786、正文 y≈83-727，
