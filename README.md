...
│  ├─ cm_run_product.py           ← 全量验证入口：按产品跑 A 档 7 类匹配（33 单条目 + DOC×3 整篇）
...
│  ├─ showcase_run.py             ← 展示用例：按票证定位真实单元→A 类别裁决→Excel（演示用全量走一遍主线）
│  ├─ cm_demo_server.py           ← demo 服务端：单网页选产品→选条目→逐类别/整目录判定
│  ├─ gen_results_xlsx.py         ← results.json → Excel（判定明细/confirmed违规/违规汇总 3 sheets）
│  ├─ gen_rule_A_doc.py           ← 重生成 A 类归类文档（可选）
│  └─ auditlib/                   ← 核心判定/解析库（仅 7 个必需模块，见下）
├─ spec/
│  ├─ spec_rules.json             ← 58 条规则知识库（机器读主数据源）
│  └─ rule_categories_A7.json     ← A 档运行配置：G1-G6 单条目 33 + 整篇 DOC×3 + B/C 排除表
└─ audit/
...
| `.tools/scripts/cm_run_product.py` | **全量验证**。读 `spec/rule_categories_A7.json` 的 7 类，对每个产品的 `pages_clean.json`（来自 m5_parse_all）里每条 API 逐类别做 LLM 判定。并发/断点续跑/熔断。产物 `results.{json,jsonl,md}`、`stats.json`、`run_log.txt`、`llm_call_log.jsonl`。 |
| `.tools/scripts/cmmatch_run.py` | 被上者 import（`task_key/load_done_keys/adjudicate_task/_annotate_printed_pages`）；也可独立跑 ModelArts 全量。 |
...
| `.tools/scripts/showcase_run.py` | 展示用例：内嵌 10 个票证（问题单「直接对比=是」实例），按端点定位 APM 真实单元→跑所属 A 类别裁决→Excel，演示端到端主线。`TICKETS` 可自行增改。 |
...
| `.tools/scripts/gen_rule_A_doc.py` | 由 spec_rules.json 重生成 A 类归类 markdown（本目录未内置 md 产物，需要时运行它现造）。 |
...
| `spec/rule_categories_A7.json` | 运行配置：哪些规则、分几类、每类哪些 —— 决定实际判定范围。 |
...
#      python .tools/scripts/m5_parse_all.py --pdfs APM      # 产出 audit/work/m5/APM/pages_clean.json
#   B. 已有 pages_clean 缓存 —— 直接把各产品清洗缓存放   audit/work/<产品>/pages_clean.json
#      （ModelArts 特例放 audit/work/pages_clean.json —— 见 cm_run_product.product_pages）
#   openpyxl 需 pip 安装（仅转 Excel 用）；demo/全量验证其余仅标准库。

# 1) 全量验证（A 档 7 类；dry-run 先探路不改产物）
python .tools/scripts/cm_run_product.py APM --out audit/cm_APM --dry-run
python .tools/scripts/cm_run_product.py APM --out audit/cm_APM --workers 10 --max-fail 12 --pdf "路径/APM.pdf"
...
- **判定范围**：`spec/rule_categories_A7.json`（类别/规则划分）。当前单条目 A=33 条 / 6 类 + 整篇 DOC×3，B/C 档排除。
...
- 未含：PDF 原文（解析入口 `m5_parse_all.py` 已有，PDF 自备放 `reference/API/`）、pages_clean 大缓存（自备或由 m5_parse_all 现造）、规则_build 全量脚本、pdf 标注所需的 PDF、`全量ABC清单/A类整合` 等 md（脚本现造）。
